...
def main():
    ...
    file_path = 'data6_resample.csv'
    predictions = prediction_main(file_path)

    # data6_resample.csvから最後の60データを読み込む
    data = pd.read_csv(file_path)
    last_60_data = data['価格'].values[-60:]

    print(f"現在の価格: {last_60_data[-1]}")
    
    # グラフのプロット
    plot_predictions(last_60_data, predictions)
    
    # 標準偏差を計算
    std_dev = np.std(last_60_data)
    # print(f"予測価格の標準偏差: {std_dev:.2f}")
    
    # 価格が下がる確率の計算
    last_actual_price = last_60_data[-1]
